...
--new procedure added after today's feedback
--get respective instructor email by addding subject name
Create procedure getInstrEmails @subject_Name varchar(255)
AS
select Instructor.instructor_Name, Instructor.instructor_Email, Subject.subject_Name
from Instructor
inner join subject
on Instructor.instructorID = Subject.instructorID
where subject_Name = @subject_Name;
EXEC getInstrEmails @subject_Name = "Database Management";
...
Alter table Grades ADD subjectName varchar(255);
Alter table TimeTable ADD timeTableID int;

-- edited
-- get timetable for failed students
Create procedure getTTforFailedStudents
As
Select TimeTable.subject_Name, TimeTable.timeTableID, TimeTable.student_Name
from TimeTable
inner join Grades 
ON TimeTable.subject_Name = Grades.student_Name
Where Grades.Percentage < 55
EXEC getTTforFailedStudents;
